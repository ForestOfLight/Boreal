...
bool TickCommandExecutor::onCommand(endstone::CommandSender &sender, const endstone::Command &command,
...
void TickCommandExecutor::query(endstone::CommandSender &sender) {
    std::string message;
...
void TickCommandExecutor::setRate(endstone::CommandSender &sender, const std::vector<std::string> &args) {
    float minRate = 0;
...
void TickCommandExecutor::freeze(endstone::CommandSender &sender) {
...
void TickCommandExecutor::unfreeze(endstone::CommandSender &sender) {
    TickSpeed::unfreeze();
    plugin.getServer().broadcastMessage("§7The game is running normally.");
}

void TickCommandExecutor::step(endstone::CommandSender &sender, const std::vector<std::string> &args) {
    int minAllowed = 1;
...
void TickCommandExecutor::sprint(endstone::CommandSender &sender, const std::vector<std::string> &args) {
    float ticks = 1.0f;
    int minAllowed = 0;
    if (args.size() > 1)
